...
class ModelClassification:
    ...
    def get_result(self):
        ...
        return self.__evaluate_model(data_loader, device, class_names)
    ...
    def __evaluate_model(self, dataloader, device, class_names):
        ...
        if len(results["labels"]) == 0:
            ...
            return "No samples found in dataloader!"

        i = 0  # index of the single sample
        ...
        result_str = (f"True: {class_names[y_true]},\n"
                    f"Top-1: {class_names[top1]} ({probs1*100:.1f}%),\n"
                    f"Top-2: {class_names[top2]} ({probs2*100:.1f}%),\n"
                    f"Top-3: {class_names[top3]} ({probs3*100:.1f}%)\n")

        p3sum = probs1 + probs2 + probs3
        if p3sum < 0.75:
            result_str += f", Top-4: {class_names[top4]} ({probs4*100:.1f}%)\n"
            p4sum = p3sum + probs4
            if p4sum < 0.85:
                result_str += f", Top-5: {class_names[top5]} ({probs5*100:.1f}%)"

        print(result_str)  # still print it to console
        return result_str  # also return it!
